...
| 9 | MCP server accessible via remote URL with response latency under 500 ms | **Latency met, hosting not** | Measured `tools/call` round-trip on loopback: **0.62, 0.78, 0.82, 0.85, 1.17 ms** over 5 samples — three orders of magnitude inside the limit, and every call resolves in memory so there is no provider-bound step. The server is **not deployed** and has no public URL; the SDK's host-header validation refuses any non-`localhost` `Host` (verified: `HTTP 403 Invalid Host`). Deploying is an owner-only action under `CLAUDE.md`'s human-only gates. |
